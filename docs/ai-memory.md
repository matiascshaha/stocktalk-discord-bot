...
| 2026-03-10 | trading | decision | Webull option execution now sizes BUY contracts by fixed-notional (`trading.fixed_notional.options`) and supports weighting-based sizing when fixed-notional is disabled; Discord passes `signal.weight_percent` into option submission for this path. | src/webull_trader.py:341; src/trading/orders/sizing.py:52; src/discord_client.py:162; tests/unit/test_webull_trader.py:403; tests/brokers/webull/integration/test_paper_trade_discord_flow.py:377 | codex | 2026-06-10 |
| 2026-03-09 | trading | decision | Discord runtime executes executable `OPTION` vehicles when `trading.options_enabled=true` via broker `place_option_order`; stock and option submits can run in the same parsed signal. | src/discord_client.py:133; src/brokerages/ports.py:8; src/brokerages/webull/broker.py:17; src/trading/contracts/options.py:1; tests/brokers/webull/integration/test_paper_trade_discord_flow.py:231 | codex | 2026-06-09 |
| 2026-03-04 | testing | decision | CI now uses one canonical test runner (`./scripts/testing/run.sh` -> `scripts.testing.runner`) and one workflow (`.github/workflows/ci.yml`); PRs run `critical`, nightly runs read-only `live-read`, AI live is manual-only. | scripts/testing/runner.py:1; scripts/testing/run.sh:1; .github/workflows/ci.yml:1 | codex | 2026-06-04 |
| 2026-03-04 | testing | decision | Paper Webull smoke credential resolution is strict: paper mode no longer falls back to production `WEBULL_APP_*` secrets; CI live-read checks use dedicated `WEBULL_TEST_*` secrets. | tests/support/webull_smoke_helpers.py:55; .github/workflows/ci.yml:40 | codex | 2026-06-04 |
| 2026-03-03 | architecture | decision | Trading runtime provider routing now supports split selection via `trading.execution_provider` and `trading.quote_provider`; mixed execution/quote runtimes are composed behind a single broker port. | src/brokerages/factory.py:31; src/brokerages/composite/broker.py:8; config/settings.py:195 | codex | 2026-06-03 |
| 2026-03-03 | config | decision | Startup validation now fails fast on unsupported or unimplemented provider selections (for example `execution_provider=public` before implementation). | src/brokerages/provider_registry.py:44; config/settings.py:277 | codex | 2026-06-03 |
| 2026-03-03 | trading | decision | Stock fixed-notional sizing now supports canonical config path `trading.fixed_notional.stocks.{enabled,amount}` with backward-compatible fallback to legacy `trading.force_default_amount_for_buys` and `trading.default_amount`. | config/settings.py:194; config/settings.py:206; config/trading.yaml:71 | codex | 2026-06-03 |
| 2026-03-03 | trading | decision | Off-hours quote-missing behavior no longer defaults to blind `$1` BUY limits; `buy_limit_price_without_quote` is now opt-in and unset by default, so execution fails safe when no executable quote is available. | config/settings.py:204; src/trading/orders/planner.py:37; tests/brokers/webull/integration/test_quote_execute_flow.py:163 | codex | 2026-06-03 |
| 2026-03-02 | ai | decision | OpenAI parser flow is now fast-stage first by default (`gpt-4.1-nano`), with full-parse fallback on ambiguity/low confidence using configurable stronger fallback model settings (`fallback_model`, `fallback_max_tokens`). | src/ai_parser.py:108; src/ai_parser.py:244; config/settings.py:146; config/trading.yaml:34 | codex | 2026-06-02 |
| 2026-03-02 | ai | decision | OpenAI full-parse model contract is `signals`-only; parser injects `source` and `meta` locally before runtime consumption. | src/providers/openai/parser_contract.py:9; src/ai_parser.py:153; src/models/parser_models.py:202 | codex | 2026-06-02 |
